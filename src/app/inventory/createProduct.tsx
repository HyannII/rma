...
export default function CreateProduct({
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^[\p{L}0-9\s]+$/u.test(productData.name)
    ) {
      newErrors.name =
    ...
    if (
      ...
      !/^[\p{L}\s]+$/u.test(productData.color)
    ) {
      newErrors.color =
    ...
    if (!productData.quantity || !/^[1-9][0-9]*$/.test(productData.quantity)) {
      newErrors.quantity =
    ...
    if (
      ...
      !/^[\p{L}\s]+$/u.test(productData.category)
    ) {
    ...
    if (
      ...
      (!/^\d+(\.\d{1,3})?$/.test(productData.customer_price) ||
        Number(productData.customer_price) < 0)
    ) {
      ...
      >
        <div className="flex flex-wrap w-2/3">
          ...
          <div className="mb-4 w-1/2 px-2">
            ...
            {isSubmitted && errorMessage.color && (
              <p className="text-red-500 text-xs mt-1">{errorMessage.color}</p>
            )}
          </div>
          <div className="mb-4 w-1/2 px-2">
            <label
        ...
        <div className="mb-4 w-2/3 px-2">
          ...
          {isSubmitted && errorMessage.category && (
            <p className="text-red-500 text-xs mt-1">{errorMessage.category}</p>
          )}
        </div>
        ...
        <div className="mb-4 w-1/2 px-2">
          ...
          {isSubmitted && errorMessage.unit && (
            <p className="text-red-500 text-xs mt-1">{errorMessage.unit}</p>
          )}
        </div>
