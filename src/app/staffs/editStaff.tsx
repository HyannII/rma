...
export default function EditStaff({
  ...
  const validateForm = () => {
    ...
    if (
      ...
      /^[\p{L}\s]+$/u.test(updatedStaff.name ?? "")
    ) {
      newErrors.name =
    ...
    if (
      ...
      !/^[\p{L}\s]+$/u.test(updatedStaff.gender)
    ) {
      newErrors.gender =
    ...
    if (
      ...
      !/^\d{4}-\d{2}-\d{2}$/.test(updatedStaff.birthday ?? "")
    ) {
      newErrors.birthday =
    ...
    if (
      ...
      !/^[0-9]{10,11}$/.test(updatedStaff.phone ?? "")
    ) {
      newErrors.phone =
        "Số điện thoại không hợp lệ. Vui lòng nhập từ 10-11 chữ số.";
    }

    // Kiểm tra số CMND/CCCD
    if (
      !updatedStaff.citizen_id?.trim() ||
      !/^[0-9]{9,12}$/.test(updatedStaff.citizen_id ?? "")
    ) {
      newErrors.citizen_id =
        "Số CMND/CCCD không hợp lệ. Vui lòng nhập từ 9-12 chữ số.";
    }
        ...
        <div className="flex flex-wrap w-2/3">
          ...
          <div className="mb-4 w-1/2 px-2">
            ...
            {isSubmitted && errorMessage.gender && (
              <p className="text-red-500 text-xs mt-1">{errorMessage.gender}</p>
            )}
          </div>
          <div className="mb-4 w-1/2 px-2">
            <label
          ...
          <div className="mb-4 w-1/2 px-2">
            ...
            {isSubmitted && errorMessage.role && (
              <p className="text-red-500 text-xs mt-1">{errorMessage.role}</p>
            )}
          </div>
